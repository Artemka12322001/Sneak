...
pygame.init()
pygame.display.set_caption("Змейка  v1.0")
pygame.font.init()
random.seed()
...
def checkCollision(posA, As, posB, Bs):
    if (
            posA.x < posB.x + Bs and posA.x + As > posB.x and posA.y < posB.y + Bs and posA.y + As > posB.y):
        return True
    return False
...
class Snake:
    ...
    def move(self):  # отвечает за движение
    ...
    def grow(self):  # отвечает за рост змейки
        last_element = len(self.stack) - 1
    ...
    def setDirection(self, direction):  # отвечает за направление
        if (self.direction == KEY["RIGHT"] and direction == KEY["LEFT"] or self.direction == KEY[
            "LEFT"] and direction == KEY["RIGHT"]):
            pass
        elif (self.direction == KEY["UP"] and direction == KEY["DOWN"] or self.direction == KEY[
            "DOWN"] and direction == KEY["UP"]):
            pass
        else:
    ...
    def draw(self, screen):  # отвечает за отрисовку
...
def respawnApple(apples, index, sx, sy):  # отвечает за появление яблока
    radius = math.sqrt(
        (SCREEN_WIDTH / 2 * SCREEN_WIDTH / 2 + SCREEN_HEIGHT / 2 * SCREEN_HEIGHT / 2)) / 2
    angle = 999
    while (angle > radius):
...
def endGame(score):  # отвечает за завершение игры
    message = game_over_font.render("Game Over :(", 1, pygame.Color("red"))
    message_play_again = play_again_font.render("Начать заново?/выйти из игры? Y/N", 1,
                                                pygame.Color(
                                                    "Orange"))
...
def drawScore(score):  # очки
    score_numb = score_numb_font.render(str(score), 1, pygame.Color("Goldenrod"))
    screen.blit(score_msg, (SCREEN_WIDTH - score_msg_size[0] - 60, 10))
    screen.blit(score_numb, (SCREEN_WIDTH - 45, 14))


def drawGameTime(gameTime):  # Время
    game_time = score_font.render("Time:", 1, pygame.Color("green"))
    game_time_numb = score_numb_font.render(str(gameTime / 1000), 1, pygame.Color("DeepPink"))
